...
//#include <linux/device.h> INUTILE ?
#include <linux/hid.h>
//#include <linux/input.h> INUTILE ?
#include <linux/module.h>
//#include <linux/spinlock.h> INUTILE ?
...
static void joycons_list_init(void) //probleme
{
	static int i = 0;
...
static void set_leds_players_by_controller_number(struct joycon_ctlr *ctlr, int controller_number) //with the player number, get and set on the good leds
{
	...
	if (ctlr->hdev->product == USB_DEVICE_ID_NINTENDO_JOYCONR && ctlr->ctlr_state == JOYCON_CTLR_STATE_READ)
		leds = ((leds << 3) & 0x8) | ((leds << 1) & 0x4) | ((leds >> 1) & 0x2) | ((leds >> 3) & 0x1); //reverse all leds
...
static void set_joycon_layout_mode(struct joycon_ctlr *ctlr, u32 btns)
{
	...
	else if ((btns & JC_BTN_L) && hdev->product == USB_DEVICE_ID_NINTENDO_JOYCONL)
	{
		if (share_temp_num > 0)
		{
			...
			if (list_controllers_devices[number - 1].joycon_L == NULL)
			{
				...
				//we have to create another thread to not stuck the controller and having a lot of events in the buffer
...
static void joycon_hid_remove(struct hid_device *hdev)
{
	...
	// if (hdev->product == USB_DEVICE_ID_NINTENDO_JOYCONR)
	// {
	// if (list_controllers_devices[0].joycon_L != NULL)
	// {
	// 	int ret;
	// 	struct joycon_ctlr *JoyconLctlr = hid_get_drvdata(list_controllers_devices[0].joycon_L);

	// 	JoyconLctlr->ctlr_state = JOYCON_CTLR_STATE_WAIT;

	// 	mutex_lock(&joycon_input_num_mutex);
	// 	mutex_lock(&ctlr->output_mutex);
	// 	ret = joycon_set_player_leds(JoyconLctlr, 0xF, 0);
	// 	//on fait clignoter les leds
	// 	if (ret)
	// 		hid_warn(list_controllers_devices[0].joycon_L->hdev, "Failed to set leds; ret=%d\n", ret);
	// 	mutex_unlock(&ctlr->output_mutex);
	// 	mutex_unlock(&joycon_input_num_mutex);
	// }
	//globalBool = false;
	// }
	// else if (ctlr == list_controllers_devices[0].joycon_L)
	// {
	// 	list_controllers_devices[0].joycon_L = NULL;
	// }
